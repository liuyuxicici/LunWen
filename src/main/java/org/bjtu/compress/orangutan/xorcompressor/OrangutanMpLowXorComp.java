package org.bjtu.compress.orangutan.xorcompressor;

import gr.aueb.delorean.chimp.OutputBitStream;

public class OrangutanMpLowXorComp {
    ...
    private int compressValue(long value) {
        ...
        if (xor == 0) {
            ...
            if (leadingZeros == storedLeadingZeros && trailingZeros >= storedTrailingZeros) {
                ...
                if (centerBits <= 8) {
                    // case 10
                    out.writeInt((((0x2 << 3) | leadingRepresentation[storedLeadingZeros]) << 3) | (centerBits & 0x7), 8);   // 2前缀 3前导 3中心位
                    out.writeLong(xor >>> (storedTrailingZeros + 1), centerBits - 1); //存储中心位
